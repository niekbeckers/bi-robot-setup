...
datafiles = dir([filename_str '_part*.mat']);   % get list of all datafiles
...
str_param_BROS1 = strrep(strsplit(get_param([modelname '/BusSelectorLoggerBROS1'],'OutputSignals'),','),'.','_').';
str_param_BROS1 = strcat(str_param_BROS1,'_BROS1');
str_param_BROS2 = strrep(strsplit(get_param([modelname '/BusSelectorLoggerBROS1'],'OutputSignals'),','),'.','_').';
...
for ii = 1:length(param_lbls)
    ...
    data.(param) = dataArray(:,param_idx{ii});
end