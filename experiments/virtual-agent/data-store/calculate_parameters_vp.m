function dataout = calculate_parameters_vp(datain, expprotocol, pairID, varargin)
...
rmse1_vp = NaN(size(connected));
rmse2 = NaN(size(connected));
rmse2_vp = NaN(size(connected));
...
for ii = 1:Nblocks % blocks
    ...
    for jj = 1:size(idx_trials,1)
        ...
        if p.Results.AlignTrials
            ...
            pos_vp = circshift(pos_vp,Nshift,1);
        end
        
        % calculate RMS for the humans
        ...
        % calculate RMS for the vp
        [rmse1_vp(jj,ii)] = calculate_performance(target(:,1:2),pos_vp(:,1:2),idx_trials,Nparts);
        [rmse2_vp(jj,ii)] = calculate_performance(target(:,3:4),pos_vp(:,3:4),idx_trials,Nparts);
...
%         fixerrors(pairID,ii,idx_connected(:,ii),idx_single(:,ii),rmse1(:,ii),rmse2(:,ii),rmse1_parts(:,ii,:),rmse2_parts(:,ii,:));
    ...
    % store virtual partner data.
    blocks(ii).rmse_vp = [rmse1_vp(:,ii) rmse2_vp(:,ii)];
    blocks(ii).rmse_vp_single = [rmse1_vp(idx_single(:,ii),ii) rmse2_vp(idx_single(:,ii),ii)];
    blocks(ii).rmse_vp_connected = [rmse1_vp(idx_connected(:,ii),ii) rmse2_vp(idx_connected(:,ii),ii)];
...
dataout.rmse_vp = [blocks(:).rmse_vp];
dataout.rmse_vp_single = [blocks(:).rmse_vp_single];
dataout.rmse_vp_connected = [blocks(:).rmse_vp_connected];
